...
**Note:**
There are other video codes that support transparency, such as `HEVC`, which might show up correctly in the preview through the ui or inside the player,
which will lose their transparency when rendering.
