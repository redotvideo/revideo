...
A `Omit<RenderSettings, 'workers'>` object with
...
[InlineConfig](https://vitejs.dev/guide/api-javascript#inlineconfig).
You can use these options to configure the server port, the cache directory, 
and more.
