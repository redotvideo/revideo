...
renderVideo('./src/project.ts', {color: 'white'}, printProgress, {
  name: 'out',
  workers: 1,
  range: [1, 3],
...
Points towards your project file. This will probably be
`./src/project.ts`.
...
`(workerId: number, progress: number) => void`:
...
FFmpeg options - is an instance of FFmpegOptions. These overwrite settings set
