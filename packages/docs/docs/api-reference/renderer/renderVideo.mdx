---
sidebar_position: 1
slug: /renderer/renderVideo
---

# renderVideo()

The `renderVideo` function lets you render (parameterized) videos in a nodejs
process. It uses a headless browser to achieve this.

## Example Usage

```tsx
import {renderVideo} from '@revideo/renderer';

renderVideo(
    './vite.config.ts',
    { color: 'white'},
    {
        name: "out",
        startInSeconds: 1,
        endInSeconds: 3,
        puppeteer: {
            args: [
                '--no-sandbox'
            ]
        },

    }
);
```

## Arguments

### configFile:

A string pointing towards your Vite config file. This will almost always be
`vite.config.ts`.

### params (optional):

Parameters / or variables passed to your video. See [here](/parameterized-video)
learn more about parameterized videos. `default = {}`

### settings (optional):

An object with the following arguments:

#### name (optional):

The file name of the video output - the video will be saved to `{name}.mp4`. `default = "project"`

#### startInSeconds (optional):

The starting second at which the video should be rendered (in case you only want to render an excerpt of the video). `default = 0`

#### endInSeconds (optional):

The starting second at which the video should be rendered (in case you only want to render an excerpt of the video). `default = Infinity`

#### puppeteer (optional):

Launch options for puppeteer - is an instance of puppeteer's
[BrowserLaunchArgumentOptions](https://pptr.dev/api/puppeteer.browserlaunchargumentoptions/)
