...
Using Emojis can cause issues as they might be rendered inconsistently across different browser versions. To get consistent results for rendering emojis, you should explicitly use a font that supports a variant of emojis you like. You can read in detail about how to use fonts [here](/custom-fonts).
...
export default makeScene2D(function* (view) {
  yield view.add(
    <Txt
      text={"Hello 🚀"}
      fontFamily={"Lexend, 'Noto Color Emoji'"}
    />
  );
