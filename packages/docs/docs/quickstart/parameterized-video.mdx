...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <Txt fontSize={1} textWrap={true} ref={textRef} fill={"white"}>Hello {name}!</Txt>,
  );
...
renderVideo('./vite.config.ts', { username: 'Mike' });
```
...
To understand how to build video projects with more complex parameters, you can check out our [reddit post reader example](/reddit-post-reader). This project fetches reddit posts and generates videos that reads them to the viewer using speech-to-text software.