...
has run, your .mp4 file will be located in the output path specified in the
arguments of `renderVideo()`, which is `output/out.mp4` by default.

```ts
import {renderVideo} from '@revideo/renderer';

renderVideo('./src/project.ts');
```

For more advanced usage, check out the
[renderVideo()](/api/renderer/renderVideo) API reference.
