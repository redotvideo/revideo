...
endpoint by passing your desired `settings` and `variables` as a parameter. This endpoint will
trigger a render process on the server, and respond with a download url once the rendering process has finished.
...
If you don't want to keep the connection to the server open during the full duration of the rendering process, you can render videos
with the `callbackUrl` parameter. This will
